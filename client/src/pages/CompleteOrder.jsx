...
export default function CompleteOrder() {
  ...
  useEffect(() => {
    ...
    if (article) {
      const rate = article.metal === 'Silver' ? settings.silver_rate_per_gram : settings.gold_rate_per_gram;
      setCart([{
  ...
  const totals = useMemo(() => {
    ...
    const preRound = round2(taxable + cgst + sgst + igst - Number(discount || 0) - Number(oldGold || 0) - advanceAmount);
  ...
  }, [cart, settings, isInterstate, documentType, discount, oldGold, advanceAmount]);
